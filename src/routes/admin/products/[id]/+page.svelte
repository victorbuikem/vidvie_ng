...
<svelte:head>
	<title>Edit {product.name} — Admin — Vidvie</title>
</svelte:head>
	...
	<Card class="mt-6">
		<form method="POST" use:enhance={() => {
			...
			<div>
				...
				<Input id="name" name="name" required value={form?.data?.name ?? product.name} />
			...
			<div>
				...
				<Textarea id="description" name="description" required rows={4} value={form?.data?.description ?? product.description} />
			...
			<div class="grid grid-cols-2 gap-4">
				<div>
					...
					<Input id="price" name="price" type="number" required value={String(product.price / 100)} />
				...
				<div>
					...
					<Input id="discountPrice" name="discountPrice" type="number" value={product.discountPrice ? String(product.discountPrice / 100) : ''} placeholder="Leave empty for no discount" />
				</div>
			</div>
			...
			<div class="grid grid-cols-2 gap-4">
				...
				<div>
					...
					<Input id="stock" name="stock" type="number" value={String(product.stock)} />
				</div>
			</div>
			...
			<div>
				<label class="flex items-center gap-2">
					<input
						...
						checked={product.featuredOnLanding}
