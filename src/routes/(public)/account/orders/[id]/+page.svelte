<script lang="ts">
	...
	const shipping = $derived(order.shippingAddress as { name: string; phone: string; address: string; city: string; state: string });
</script>

<svelte:head>
	<title>Order {order.id.slice(0, 12)} — Vidvie</title>
</svelte:head>

<div class="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
	<div class="flex items-center gap-3">
		<a href="/account/orders" class="text-surface-400 hover:text-surface-600">
			<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
				<path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
			</svg>
		</a>
		<h1 class="font-heading text-2xl font-medium text-surface-900">Order Details</h1>
	</div>
	...
	<Card class="mt-6" padding={false}>
		...
		<div class="flex justify-end px-6 py-4">
			<p class="text-lg font-bold text-surface-900">Total: {formatPrice(order.total)}</p>
		</div>
	</Card>
</div>
